...
public class HeapFileEncoder {
    ...
    public static void convert(Object[][] tuples, Type[] typeAr, File out, int npagebytes)
            throws IOException, SchemaException {
        File tempInput = File.createTempFile("tempTable", ".txt");
        tempInput.deleteOnExit();
        BufferedWriter bw = new BufferedWriter(new FileWriter(tempInput));
        for (Object[] tuple : tuples) {
            if (tuple.length != typeAr.length) {
                throw new SchemaException("Tuple doesn't have right number of fields");
            }
            for (int i = 0; i < typeAr.length; i++) {
                Object dataVal = tuple[i];
                Class dataCls = typeAr[i].getUnderlyingCls();
                if (!dataCls.isInstance(dataVal)) {
                    throw new SchemaException("Invalid data type");
                }
            }
            String lineStr = String.join(
                    ",",
                    Arrays.stream(tuple).map(Object::toString).toArray(String[]::new)
            );
            bw.write(lineStr);
            bw.write("\n");
        }
        bw.close();
        convert(tempInput, out, npagebytes, typeAr.length, typeAr);
    }
